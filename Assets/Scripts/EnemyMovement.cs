...
public class EnemyMovement : MonoBehaviour
{
    ...
    // Lấy dữ liệu từ database khi bắt đầu nếu có
        if (dbManager != null)
        {
            DatabaseManager.SoldierEnemy data = dbManager.GetSoldierEnemyById(id); // mÀ gọi ở đây không được
            if (data != null)
            {
                healthBar.SetMaxHealth(data.HealthEnemy1);
                damage = data.DamageEnemy1;
            }
        }
    }
